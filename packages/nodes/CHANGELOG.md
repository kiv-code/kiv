...
- Fix `sizeField()` dropping `showIf`. `SizeFieldOptions` never declared the
  option and the returned descriptor never forwarded it, so a size field asking to
  be conditional was silently unconditional. Four `agenda-item` fields were
  affected — `speakerLabelFontSize`, `speakerNameFontSize`,
  `speakerTitleFontSize`, `speakerCompanyFontSize` — all of which meant to hide
  themselves when `hasSpeaker` is off and instead showed up in the inspector at
  all times.
