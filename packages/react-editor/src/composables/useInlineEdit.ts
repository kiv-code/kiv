...
export function useInlineEdit(store: EditorStore) {
	// `store` is a fresh plain object every render (unlike Vue's ref-boxed
	// store, where `.value` always reads live) — commit()'s DOM event
	// listeners are attached once per `activate()` call and can outlive
	// several re-renders, so they read `storeRef.current` (always the latest
	// store) rather than closing over whatever `store` was at activation time.
	const storeRef = useRef(store);
	storeRef.current = store;

	const currentElRef = useRef<HTMLElement | null>(null);
	const currentHtmlRef = useRef(false);
	...
	function showToolbar(el: HTMLElement) {
		...
		setToolbarTop(Math.max(8, rect.top - 40));
	...
	function commit(
		...
		if (newValue !== originalContentRef.current) {
	...
	function activate(
		...
		originalContentRef.current = getContent(el, html);
	...
	function execFormat(command: string, value?: string) {
		if (!currentElRef.current || !currentHtmlRef.current) return;
	...
	function onCanvasDblClick(e: MouseEvent) {
		...
		storeRef.current.select(nodeId);
		e.preventDefault();
		e.stopPropagation();
